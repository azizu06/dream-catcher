...
const createRateLimitWindowMs = Number(
  process.env.DREAM_CREATE_RATE_LIMIT_WINDOW_MS || 15 * 60 * 1000,
);
...
router.post("/", createDreamLimiter, async (req, res) => {
  ...
  if (dailyUsage.count >= dailyDreamLimit) {
    return res.status(429).json({
      error:
        "The daily dream interpretation limit has been reached. Please try again tomorrow.",
      type: "daily_limit",
    });
  }
